...
if os.environ.get("LOCAL_READER_TTS_MODEL_VERSION"):
    ...
    TTS_ENGINE_VERSION = "vieneu-tts-0.3b-full-fp32"
    ...
    if isinstance(doc, dict):
        ...
        if locked_engine == "vieneu" or doc_has_vieneu_audio(doc):
            return {
                ...
                "audioEngine": "vieneu",
                "audioModelVersion": VIENEU_MODEL_VERSION,
            }
...
def project_audio_cache_key(text, voice="Ly"):
    clean = prepare_tts_text(text)
    return f"vieneu|{TTS_ENGINE_VERSION}|{VIENEU_AUDIO_SPEED}|{voice or 'Ly'}|{stable_hash_text(clean)}"
...
def sanitize_project_store_for_write(data):
    ...
    for incoming in data.get("docs") if isinstance(data.get("docs"), list) else []:
        ...
        if vieneu_entries:
            ...
            sanitize_project_audio_items(doc, vieneu_entries, "Ly", "Ly [VieNeu]", VIENEU_MODEL_VERSION)
            docs.append(doc)
            continue
...
def normalize_prepare_jobs(jobs):
    ...
    for job in jobs if isinstance(jobs, list) else []:
        ...
        if not text:
            continue
        voice = str(job.get("voice") or "Ly").strip() or "Ly"
...
def cached_audio_url_for_job(job, manifest=None):
    ...
    if not text:
        return ""
    voice = normalize_tts_voice(job.get("voice") or "Ly")
...
def prepared_doc_for_auto_queue(doc):
    ...
    for job in jobs:
        ...
        if cached_url:
            ...
            clean_items.append({
                ...
                "url": cached_url,
                "voice": job.get("voice") or "Ly",
                "voiceLabel": "Ly [VieNeu]",
    ...
    prepared["audioVoice"] = "Ly"
    prepared["audioVoiceLabel"] = "Ly [VieNeu]"
...
def run_background_project_prepare(doc_id, jobs, audio_signature):
    ...
    try:
        ...
        def synth_one(job):
            filename = synthesize_preferred_wav_sync(job["text"], job.get("voice") or "Ly")
...
async def edge_voice_list():
    voices = [
        {"name": "Ly", "display": "Ly [VieNeu]"},
        {"name": "Ngoc", "display": "Ngọc [VieNeu]"},
        {"name": "Doan", "display": "Đoan [VieNeu]"},
    ]
    return [{
...
def normalize_tts_voice(value, default="Ly"):
...
def synthesize_preferred_wav_sync(text, voice=None):
    ...
    requested_voice = normalize_tts_voice(voice or "Ly")
